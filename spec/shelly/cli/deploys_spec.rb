...
describe Shelly::CLI::Deploys do
  ...
  describe "#list" do
    ...
    it "should exit if user doesn't have access to cloud in Cloudfile" do
      @client.stub(:deploy_logs).and_raise(Shelly::Client::NotFoundException.new({"message" => "Couldn't find Cloud with"}))
  ...
  describe "#show" do
    ...
    context "log not found" do
      it "should exit 1 with message" do
        exception = Shelly::Client::NotFoundException.new({"message" => "Couldn't find Log with"})
        @client.stub(:deploy_log).and_raise(exception)
